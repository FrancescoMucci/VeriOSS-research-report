...
/// @title Version E of contract responsible for the partial rewarding process of the VeriOSS platform
contract ChallengeReward is usingProvable {
    ...
    function timedTransitionTo (States _nextState, uint _deadline,  string memory _msg) 
        private 
    {
        if (now >= _deadline) {
    ...
    modifier timedTransitions() {
        if (state == States.QRY_SNT) {
            ...
            timedTransitionTo(States.EXP, expire, "Challenge expired.");
        }
        _;
    }
    ...
    function secureTransfer() private  {
